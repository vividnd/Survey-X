-- Add permissive policies to all tables with RLS enabled but no policies
DO $$
DECLARE
    table_record RECORD;
    policy_name TEXT;
...
BEGIN
    ...
    LOOP
        -- Create policy name
        policy_name := 'Allow all operations on ' || table_record.table_name;
        
        -- Create the policy
        EXECUTE 'CREATE POLICY "' || policy_name || '" ON ' || 
